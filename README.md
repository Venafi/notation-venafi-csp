...
- Run a local instance of the [CNCF Distribution Registry][cncf-distribution], with [ORAS Artifacts][artifact-manifest] support.
  ```bash
  docker run -d -p ${PORT}:5000 ghcr.io/oras-project/registry:v1.0.0-rc.3
  ```
- Build and Push the `net-monitor` software

```bash
docker build -t localhost:5005/net-monitor:v1 https://github.com/wabbit-networks/net-monitor.git#main
docker push localhost:5005/net-monitor:v1
```
- List the image, and any associated signatures.  At this point there shouldn't be any signatures
```bash
notation list --plain-http localhost:5005/net-monitor:v1 
```
...
```bash
notation sign --key "vsign-rsa2048-cert" localhost:5005/net-monitor:v1
```
- Verify the container image signature
```bash
notation verify --plain-http localhost:5005/net-monitor:v1
```
```bash
